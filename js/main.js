// Pulsante WhatsApp fisso (iniettato su tutte le pagine)
(function () {
  if (document.querySelector('.wa-float')) return;
  const isEnglish = document.documentElement.lang === 'en';
  const a = document.createElement('a');
  a.className = 'wa-float';
  a.href = 'https://wa.me/393513746102?text=' +
    encodeURIComponent(isEnglish ? 'Hello, I would like to book an ultrasound scan.' : 'Salve, vorrei prenotare un’ecografia.');
  a.target = '_blank';
  a.rel = 'noopener noreferrer';
  a.referrerPolicy = 'no-referrer';
  a.setAttribute('aria-label', isEnglish ? 'Contact us on WhatsApp' : 'Scrivici su WhatsApp');
  a.innerHTML =
...
(function () {
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && mobileNav.classList.contains('open')) setMenuOpen(false, true);
  });
